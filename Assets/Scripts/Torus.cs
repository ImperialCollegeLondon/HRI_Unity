...
public class Torus : MonoBehaviour
{
    ...
    public float tubeRadius = 0.1f;//0.1f
    ...
    public void torus(GameObject torusMesh, float segmentRadius, float tubeRadius, int segments, int tubes, Color color)
    {
        // Total vertices
        ...
        ;
        MeshFilter mFilter = torusMesh.GetComponent(typeof(MeshFilter)) as MeshFilter;
        mFilter.mesh = mesh;
    ...
    void Start()
    {
        torusMesh = new GameObject();
        torusMesh.transform.name = "TorusMesh";
    ...
    void Update()
    {
        ...
        { // Chech if the parameters changed to improve performance
            segmentRadius = transform.localScale.x;
            tubeRadius = transform.localScale.y;
            tubes = Mathf.RoundToInt(transform.localScale.z);
            torus(torusMesh, segmentRadius, tubeRadius, Mathf.RoundToInt(32 * (1 + segmentRadius / 10.0f)), tubes, color);
