...
public class CmdPathOverlay : ROSBridgeSubscriber {

	//Path information
	public static Vector3[] overlayCmdRosPos;
	public static Quaternion[] overlayCmdRosQuat;
	...
	public static void CallBack(ROSBridgeMsg msg) {
		...
		overlayCmdRosReceived = true;
		//Debug.Log ("Commanded overlay path received!");

		//Declare variables
		PathMsg pathmsg_ = (PathMsg)msg;

		//Initialise length based on number of points in the path
		overlayCmdRosPos = new Vector3[pathmsg_.getPathLength ()];
		overlayCmdRosQuat = new Quaternion[pathmsg_.getPathLength ()];
		...
		for (var i = 0; i < pathmsg_.getPathLength (); i++) {
			...
			overlayCmdRosPos [i] = new Vector3 (px, py, pz);
			overlayCmdRosQuat [i] = new Quaternion (qx, qy, qz, qw);
			//Debug.Log ("Overlay Command Path " + i + "Position: " + wayPointPos [i] + "Quaternion: " + wayPointRot [i]);
		}
	}
}
